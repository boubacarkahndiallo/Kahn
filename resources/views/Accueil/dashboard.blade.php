...
@section('content')
    ...
    <div class="container py-2">
        ...
        @push('scripts')
            ...
            <script>
                (function() {
                    // Préparer les données depuis PHP
                    const labels7 = @json($labels7 ?? []);
                    // Forcer la conversion en nombres (évite les strings provenant du back)
                    const salesData = (@json(array_values((array) ($salesByDay ?? []))) || []).map(v => Number(v) || 0);
                    const ordersData = (@json(array_values((array) ($ordersByDay ?? []))) || []).map(v => Number(v) || 0);
                    const statutLabels = @json(array_keys($statuts ?? []));
                    const statutValues = (@json(array_values($statuts ?? [])) || []).map(v => Number(v) || 0);
                    const topLabels = @json(array_keys($topProducts ?? []));
                    const topValues = (@json(array_values($topProducts ?? [])) || []).map(v => Number(v) || 0);

                    // Helper pour créer un chart en capturant les erreurs
                    ...
                    safeCreateChart('chartVentes', () => ({
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                data: salesData,
                                borderColor: '#1c911e',
                                backgroundColor: 'rgba(28,145,30,0.08)',
                                tension: 0.35,
                                fill: true,
                                pointRadius: 4
                            }]
                    ...
                    // Chart statuts (doughnut)
                    safeCreateChart('chartStatuts', () => ({
                        type: 'doughnut',
                        data: {
                            labels: statutLabels,
                            datasets: [{
                                data: statutValues,
                                backgroundColor: ['#1c911e', '#ffc107', '#dc3545', '#6c757d'],
                            }]
                        },
                        options: {
                            responsive: true
                    ...
                    // Chart commandes (bar)
                    safeCreateChart('chartCommandes', () => ({
                        type: 'bar',
                        data: {
                            labels: labels7,
                            datasets: [{
                    ...
                    // Chart top produits (horizontal bar)
                    safeCreateChart('chartProduits', () => ({
                        type: 'bar',
                        data: {
                            labels: topLabels,
                            datasets: [{
